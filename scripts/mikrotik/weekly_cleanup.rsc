# WiPay Weekly Cleanup Script (Mikhmon-Aware)
# Removes users with "Weekly" profile if their expiration date in comment has passed
...
:foreach i in=[/ip hotspot user find where profile="Weekly"] do={
    ...
    :if ($slashPos >= 3) do={
        ...
        :if ([:pick $usrComment ($slashPos + 3)] = "/") do={
            ...
            :if (($expd < $today) or ($expd = $today and $expt < $curtime)) do={
                :log info "WiPay Cleanup: Removing expired Weekly user $usrName"
