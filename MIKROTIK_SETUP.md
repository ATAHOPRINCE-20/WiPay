...
13. **Enable MAC Cookie Auto-Login (Remember Returning Devices)**:
    - Go to **IP** -> **Hotspot** -> **Server Profiles** tab.
    - Double-click `hsprof1` (or your profile).
    - Go to the **Login** tab.
    - Check **HTTP PAP**, **Cookie**, and **MAC Cookie**.
    - Set **HTTP Cookie Lifetime**: `7d 00:00:00` (7 days).
    - Click **Apply** -> **OK**.
    - _Or run in Terminal_: `/ip hotspot profile set [find] login-by=http-pap,cookie,mac-cookie cookie-lifetime=7d`
