...
export default function Withdraw() {
  ...
  const fetchStatsAndHistory = async () => {
    try {
      ...
      setStats({
        net_balance: fin.withdrawable_balance ?? fin.net_balance ?? 0,
        total_revenue: fin.online_net_revenue ?? fin.net_revenue ?? 0,
        total_withdrawn: fin.total_withdrawn ?? 0,
        pending_withdrawals: fin.pending_withdrawals ?? 0
      })
    ...
    try {
      ...
      const pollInterval = setInterval(async () => {
        ...
        if (attempts >= maxAttempts) {
          ...
          setFeedback({
            success: true,
            message: 'Withdrawal request submitted. Gateway confirmation is pending; please check your history list below.'
          })
          setStep('feedback')
          fetchStatsAndHistory()
        }
      ...
      <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2 self-end sm:self-auto w-full sm:w-auto">
            ...
            <button onClick={fetchStatsAndHistory} className="p-2 border border-gray-200 rounded-lg text-gray-500 hover:bg-gray-50 transition-colors" title="Sync & Refresh Gateway Status">
                ...
                <div className="text-center space-y-4 py-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-gray-500 max-w-xs mx-auto">
                      Please wait while we confirm the payout status with Relworx & Mobile Money network.
                    </p>
                  </div>
                </div>
              )}
