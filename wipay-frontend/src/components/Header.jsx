...
import { Search, Settings, Calendar, Filter } from 'lucide-react'
...
export default function Header({ expiry }) {
  const { admin } = useAuth()
    ...
    <header className="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-6 sticky top-0 z-20">
      ...
      <div>
        <h1 className="text-lg font-semibold text-gray-900">
          {greeting.text}, {name} <span>{greeting.emoji}</span>
        </h1>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative hidden sm:block">
          ...
          <input
            type="text"
            placeholder="Search"
            className="pl-8 pr-4 py-1.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-400 w-48"
          />
        ...
        {/* Filters */}
        <button className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-xs text-gray-500 hover:bg-gray-100 transition">
