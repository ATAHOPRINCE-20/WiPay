...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  if (admin?.role === 'agent') {
    return (
      <>
        {isOpen && <div className="fixed inset-0 z-20 bg-black/50 md:hidden" onClick={closeSidebar} />}
        <aside className={`fixed inset-y-0 left-0 z-30 w-56 bg-white border-r border-gray-100 flex flex-col transform transition-transform duration-200 ease-in-out md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex items-center justify-between px-5 py-5 border-b border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div className="overflow-hidden">
                <p className="text-sm font-bold text-gray-900 leading-none truncate">{admin?.username || 'UGPAY Agent'}</p>
                <p className="text-[10px] text-emerald-600 font-semibold mt-0.5">Reseller Agent</p>
              </div>
            </div>
            <button className="p-1 text-gray-400 hover:bg-gray-100 rounded-lg md:hidden" onClick={closeSidebar}>
              <X className="w-5 h-5" />
            </button>
          </div>

          <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-2">
            <p className="px-2 mb-1 text-[10px] font-semibold uppercase tracking-widest text-gray-400">AGENT MENU</p>
            <NavLink
              to="/agent-portal"
              onClick={closeSidebar}
              className={({ isActive }) =>
                `flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-semibold transition-colors ${
                  isActive ? 'bg-emerald-500 text-white shadow-sm' : 'text-gray-600 hover:bg-emerald-50 hover:text-emerald-700'
                }`
              }
            >
              <ShoppingBag className="w-4 h-4 flex-shrink-0" />
              <span>Voucher Sales POS</span>
            </NavLink>
          </nav>
                  ...
                  <li key={label} className={`relative group/item flex items-center ${isLocked ? 'opacity-40 pointer-events-none' : ''}`}>
                    {isExternal ? (
                      <a
                        href={
                          label === 'Check Site'
                            ? `/captive-portal?slug=${admin?.portal_slug || ''}`
                            : '#'
                        }
                        target="_blank"
